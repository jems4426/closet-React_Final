...
const Dashboard = ({ category: pageCategory }) => {
  ...
  const mainCategories = [
    ...
    { id: 2, name: 'Men', count: products.filter(p => p.mainCategory === 'Men').length },
    { id: 3, name: 'Women', count: products.filter(p => p.mainCategory === 'Women').length }
  ];
  const subCategories = [
    { id: 1, name: 'All', count: products.length },
    { id: 2, name: 'TOP', count: products.filter(p => p.subCategory === 'TOP' || p.category === 'TOP').length },
    { id: 3, name: 'LOWER', count: products.filter(p => p.subCategory === 'LOWER' || p.category === 'LOWER').length },
    { id: 4, name: 'SAREE', count: products.filter(p => p.subCategory === 'SAREE' || p.category === 'SAREE').length }
  ];
  ...
  const filteredProducts = useMemo(() => {
    return products.filter((product) => {
      const mainCategoryMatch = pageCategory ? product.mainCategory === pageCategory : (selectedMainCategory === 'All' || product.mainCategory === selectedMainCategory);
      const subCategoryMatch = selectedSubCategory === 'All' || product.subCategory === selectedSubCategory || product.category === selectedSubCategory;
  ...
  `}
    ...
    `}>
                    ...
                    <div className={`${viewMode === 'list' ? 'flex-1' : ''}`}>
                      ...
                      <p className="text-sm text-gray-500 mb-2">{product.mainCategory} - {product.subCategory || product.category}</p>
          ...
          ${product.stock > 0
                        ...
                        >
                          ...
                          <span>{product.stock > 0 ? (product.sizes?.length > 0 ? 'Select' : 'Add') : 'Out of Stock'}</span>
                        </button>
                      </div>
                    </div>
                  </div>

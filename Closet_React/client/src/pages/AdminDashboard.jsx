            ...
            <div className="rounded-2xl border border-white/30 bg-white/40 backdrop-blur-xl shadow-[0_10px_30px_rgba(0,0,0,0.08)] overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50/50">
                  <tr>
                    {['Image', 'Title', 'Main Category', 'Sub Category', 'Price', 'Sizes', 'Stock', 'Actions'].map(header => (
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="rounded-3xl border border-white/40 bg-white backdrop-blur-xl shadow-[0_20px_40px_rgba(0,0,0,0.15)] max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              <div className="p-8">
                ...
                <pre className="text-sm bg-gray-50 p-4 rounded-xl overflow-auto">{JSON.stringify(viewingOrder, null, 2)}</pre>
              </div>
            </div>
          </div>
        )}
  ...
  return (
    <form className="space-y-6" onSubmit={submit}>
      ...
      <div>
        ...
        <input type="file" accept="image/*" multiple onChange={(e)=>setForm({...form,images:[...e.target.files]})} required={!initial} className="border border-gray-300 rounded-xl px-4 py-3 w-full focus:ring-2 focus:ring-rose-500 focus:border-rose-500 transition-all" />
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
